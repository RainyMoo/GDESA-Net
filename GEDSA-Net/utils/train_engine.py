...
def train_engine(__C, dataset, dataset_eval=None):
    ...
    for epoch in range(start_epoch, __C.MAX_EPOCH):
        ...
        for step, (
            ...
            grid_bbox_feat_iter = grid_bbox_feat_iter.cuda()
            w_feat_iter = w_feat_iter.cuda()
            h_feat_iter = h_feat_iter.cuda()
            #spa_graph_iter = spa_graph_iter.cuda()
            region_aligns_iter = region_aligns_iter.cuda()
            region_iou_iter = region_iou_iter.cuda()
            ques_ix_iter = ques_ix_iter.cuda()
            attr_obj_iter = attr_obj_iter.cuda()
            ...
            for accu_step in range(__C.GRAD_ACCU_STEPS):
                ...
                sub_bbox_feat_iter = \
                    bbox_feat_iter[accu_step * __C.SUB_BATCH_SIZE:
                                  (accu_step + 1) * __C.SUB_BATCH_SIZE]
                sub_grid_bbox_feat_iter = \
                    grid_bbox_feat_iter[accu_step * __C.SUB_BATCH_SIZE:
                                  (accu_step + 1) * __C.SUB_BATCH_SIZE]
                sub_w_feat_iter = \
                    w_feat_iter[accu_step * __C.SUB_BATCH_SIZE:
                                  (accu_step + 1) * __C.SUB_BATCH_SIZE]
                sub_h_feat_iter = \
                    h_feat_iter[accu_step * __C.SUB_BATCH_SIZE:
                                  (accu_step + 1) * __C.SUB_BATCH_SIZE]             
                sub_region_aligns_iter = \
                    region_aligns_iter[accu_step * __C.SUB_BATCH_SIZE:
                                  (accu_step + 1) * __C.SUB_BATCH_SIZE]
                sub_region_iou_iter = \
                    region_iou_iter[accu_step * __C.SUB_BATCH_SIZE:
                                  (accu_step + 1) * __C.SUB_BATCH_SIZE]
                sub_ques_ix_iter = \
                    ques_ix_iter[accu_step * __C.SUB_BATCH_SIZE:
                                 (accu_step + 1) * __C.SUB_BATCH_SIZE]
                sub_attr_obj_iter = \
                    attr_obj_iter[accu_step * __C.SUB_BATCH_SIZE:
                                 (accu_step + 1) * __C.SUB_BATCH_SIZE]
                sub_ans_iter = \
                    ans_iter[accu_step * __C.SUB_BATCH_SIZE:
                             (accu_step + 1) * __C.SUB_BATCH_SIZE]
                
                if __C.ITC_LOSS == 'True':
                    #pred, region, grid, region_pred, grid_pred, itc_loss1, itc_loss2, local_align_loss = net(
                    pred, local_align_loss = net(
                        sub_frcn_feat_iter,
                        sub_grid_feat_iter,
                        sub_bbox_feat_iter,
                        sub_grid_bbox_feat_iter,
                    ...
                    loss_main = loss_fn(loss_item[0], loss_item[1])
                    loss = loss_main + __C.LOSS_LAMDA*local_align_loss
            ...
            if __C.VERBOSE:
                ...
                lrs = [g['lr'] for g in optim.optimizer.param_groups]
                print("\r[Version %s][Model %s][Dataset %s][Epoch %2d][Step %4d/%4d][%s] Loss: %.4f, Lr: %s" % (
                    __C.VERSION,
                    __C.MODEL_USE,
                    __C.DATASET,
                    epoch + 1,
        ...
        logfile.write(
            ...
            # ', Lr: ' + str(optim._rate) + '\n' +
            ', Lr: ' + str(optim.optimizer.param_groups[0]["lr"]) + '\n' +
